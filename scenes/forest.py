"""Forest scene - fireflies floating in the forest"""
from PIL import Image, ImageDraw
from scenes.common import *
import random, math

W2, H2 = 320, 240
_bg = Image.open("/home/pi/scenes/forest_bg.png").convert("RGB").resize((W2, H2))
...
class Firefly:
    def __init__(self):
        ...
        self.y = random.uniform(H2 // 4, H2 - 30)
    ...
    def update(self):
        ...
        self.x += self.vx
        self.y += self.vy
        if self.x < 0 or self.x > W2: self.vx *= -1
        if self.y < H2//4 or self.y > H2-30: self.vy *= -1
    def draw(self, img):
        alpha = int((math.sin(self.phase) + 1) / 2 * 200 + 30)
        ff = Image.new("RGBA", (W2, H2), (0, 0, 0, 0))
        d = ImageDraw.Draw(ff)
        d.ellipse([self.x-3, self.y-3, self.x+3, self.y+3], fill=(200, 255, 100, alpha))
        d.ellipse([self.x-1, self.y-1, self.x+1, self.y+1], fill=(240, 255, 180, min(255, alpha+80)))
        img.alpha_composite(ff)

_flies = [Firefly() for _ in range(25)]
_scroll_x = 0
_scroll_pause = 0
_last_song = ""
...
def draw_frame(image, now, mpd, scroll_x):
    ...
    bar2 = Image.new("RGBA", (W2, 26), (10, 25, 10, 148))
    ...
    if sw <= area_w:
        ...
        off = Image.new("RGB", (loop_w*2, 18), (10, 25, 10))
